...
// This uses memoziation:
// Each feasible path is stored, but any subpath of a feasible path is removed first to save space and avoid double checking)
public class DriverConsumerDP implements Runnable {
    ...
    public DriverConsumerDP(Algorithms alg, Driver driver) {
        Alg = alg;
        this.driver = driver;
    }
                ...
                for (int index = 0; index < passengerList.size(); index++) {        // grow each match with a passenger
                    ...
                    if (matchAndPaths != null) {
                        ...
                        if (driver.getMatches().size() >= SimulationParameters.maxNumMatchesPerDriver) {
                            //System.out.println("Driver "+driver.getID()+" with "+driver.getMatches().size()+" matches took "+(System.currentTimeMillis()- startTime)+" milliseconds to finish.");
